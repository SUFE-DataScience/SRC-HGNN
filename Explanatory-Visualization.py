...
def add_real_fake_legend(ax, loc="best", fontsize=15):
    handles = [
...
def scatter_embedding(ax, Z2: np.ndarray, y: np.ndarray, title: str, show_legend: bool = False):
    ...
    if m0.any():
        ax.scatter(Z2[m0, 0], Z2[m0, 1], s=8, alpha=0.80, color=COLORS["real"], label="Real")
    if m1.any():
        ax.scatter(Z2[m1, 0], Z2[m1, 1], s=8, alpha=0.80, color=COLORS["fake"], label="Fake")
...
def make_fig1_tSNE_2x1(X_unw, y_unw, X_w, y_w):
    idxu_ts = downsample_indices_by_class(y_unw, MAX_TSNE_PER_CLASS)
    idxw_ts = downsample_indices_by_class(y_w, MAX_TSNE_PER_CLASS)

    fig, axes = plt.subplots(1, 2, figsize=(12.0, 6.0), constrained_layout=True)

    Z_left = reduce_2d("TSNE", X_w[idxw_ts])
    scatter_embedding(axes[0], Z_left, y_w[idxw_ts], title="")
    axes[0].text(0.5, -0.08, "(a) MODE-base", transform=axes[0].transAxes,
                 ha="center", va="top", fontsize=16)

    Z_right = reduce_2d("TSNE", X_unw[idxu_ts])
    scatter_embedding(axes[1], Z_right, y_unw[idxu_ts], title="")
    axes[1].text(0.5, -0.08, "(b) MODE-WT", transform=axes[1].transAxes,
                 ha="center", va="top", fontsize=16)

    save_show(fig, "FIG1_TopoFeatures_tSNE_2x1.pdf")
...
def plot_fig2_ccdf_2x5(X_unw_raw, y_unw, X_w_raw, y_w):
    ...
    for i, f in enumerate(NODE_FEATURES):
        ax = axes[i // 5, i % 5]
        title_en = feature_names_en.get(f, f)

        ax.set_title(title_en, pad=6, fontsize=14)
        ax.set_box_aspect(1)

        for (Xraw, y, ls, tag) in [
            (X_unw_raw, y_unw, "-", "MODE-base"),
            (X_w_raw, y_w, "--", "MODE-WT"),
        ]:
            xr = Xraw[y == 0, i]
            xf = Xraw[y == 1, i]
            ...
            xs, yy = ccdf_xy(xr)
            if xs is not None:
                ax.plot(xs, yy, linestyle=ls, linewidth=1.8, color=COLORS["real"], label=f"Real-{tag}")

            xs, yy = ccdf_xy(xf)
            if xs is not None:
                ax.plot(xs, yy, linestyle=ls, linewidth=1.8, color=COLORS["fake"], label=f"Fake-{tag}")
...
def draw_coreperi_schematic(
    ...
    nx.draw_networkx_edges(H, pos, ax=ax, width=0.05, alpha=0.2, edge_color=COLORS["edge"])
    ...
    if add_legend:
        add_real_fake_legend(ax, loc="lower left", fontsize=13)

def plot_fig7_coreperi(Guu_unw: nx.Graph, Guu_w: nx.Graph, user_labels: pd.Series):
    fig, axes = plt.subplots(1, 2, figsize=(10.0, 5.2))
    plt.subplots_adjust(wspace=0.15, bottom=0.18)

    draw_coreperi_schematic(axes[0], Guu_unw, user_labels, weighted=False, add_legend=True)
    draw_coreperi_schematic(axes[1], Guu_w, user_labels, weighted=True, add_legend=True)

    axes[0].text(
        0.5, -0.08, "(a) Core Periphery (MODE-base)",
        transform=axes[0].transAxes,
        ha="center", va="top", fontsize=18
    )
    axes[1].text(
        0.5, -0.08, "(b) Core Periphery (MODE-WT)",
...
def main():
    ...
    tasks_total = 8
    with tqdm(total=tasks_total, desc="Total progress", ncols=100) as pbar:
        ...
        if MAKE_FIG1_EMB_2x3:
            make_fig1_tSNE_2x1(X_unw, y_unw, X_w, y_w)
        pbar.update(1)
